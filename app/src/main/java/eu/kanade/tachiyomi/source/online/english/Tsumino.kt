...
class Tsumino(delegate: HttpSource, val context: Context) :
    ...
    override val domainName: String = "tsumino"
    override fun canOpenUrl(uri: Uri): Boolean {
        val lcFirstPathSegment = uri.pathSegments.firstOrNull()?.lowercase(Locale.ROOT) ?: return false
        return lcFirstPathSegment == "read" || lcFirstPathSegment == "book" || lcFirstPathSegment == "entry"
    }
