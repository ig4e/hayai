    ...
    override fun getDrawable(source: String?): Drawable {
        ...
        if (source.startsWith("data:")) {
            try {
                ...
                Logger.d { "Failed to decode base64 image: ${e.message}" }
            }
            return wrapper
        }

        if (source.startsWith("hayai-novel-image://")) {
            scope.launch(Dispatchers.IO) {
                try {
                    val imagePath = android.net.Uri.decode(source.removePrefix("hayai-novel-image://"))
                    ...
                    Logger.d { "Failed to load custom novel image: ${e.message}" }
                }
            }
            return wrapper
        }
        ...
        if (!source.startsWith("http://") && !source.startsWith("https://") && !source.startsWith("//")) {
            Logger.d { "Skipping non-URL image source: $source" }
            return wrapper
        }
        ...
        scope.launch {
            try {
                ...
                Logger.d { "Failed to load image in novel reader: $imageUrl - ${e.message}" }
            }
        }
 ...
 */
class NovelViewer(val activity: ReaderActivity) : BaseViewer, TextToSpeech.OnInitListener {
    ...
    private fun loadNextChapterIfAvailable() {
        val anchor = loadedChapters.getOrNull(currentChapterIndex)?.chapter
            ?: currentChapters?.currChapter ?: run {
                Logger.e { "NovelViewer: loadNext failed, no anchor (loadedCount=${loadedChapters.size})" }
                showInlineError("No anchor chapter for infinite scroll", isPrepend = false)
                return
            }

        if (isLoadingNext) {
            Logger.d { "NovelViewer: loadNext ignored, already loading" }
            return
        }
        isLoadingNext = true
        Logger.d {
        ...
        scope.launch {
            try {
                val preparedChapter = activity.viewModel.prepareNextChapterForInfiniteScroll(anchor) ?: run {
                    Logger.w { "NovelViewer: No next chapter after ${anchor.chapter.name}" }
                ...
                val page = preparedChapter.pages?.firstOrNull() as? ReaderPage ?: run {
                    Logger.e { "NovelViewer: No page in prepared next chapter" }
                    showInlineError("No page in next chapter", isPrepend = false)
                    return@launch
                }
                val loader = page.chapter.pageLoader ?: run {
                    Logger.e { "NovelViewer: No loader for next chapter" }
                ...
                val loaded = try {
                    ...
                    Logger.e { "NovelViewer: Timed out loading next chapter page after 30s" }
                    ...
                    Logger.d { "NovelViewer: loadNext cancelled" }
                    false
                } catch (e: Exception) {
                    Logger.e { "NovelViewer: Error loading next chapter page: ${e.message}" }
                ...
                Logger.d { "NovelViewer: appending chapter ${preparedChapter.chapter.id}" }
    ...
    private fun showInlineLoading(isPrepend: Boolean) {
        ...
        inlineLoadingView = TextView(activity).apply {
            text = activity.getString(MR.strings.loading)
            textSize = 14f
    ...
    private fun observePreferences() {
        ...
        scope.launch {
            merge(
                ...
                preferences.novelMarginsCropped.changes(),
                preferences.novelFontColor.changes(),
                preferences.novelBackgroundColor.changes(),
            ).drop(12) // Drop initial emissions from all 12 preferences
                .collect {
        ...
        // When the engine changes, the TTS instance must be recreated — voices and supported
        // features can differ across engines.
        scope.launch {
            preferences.novelTtsEngine.changes().drop(1).collectLatest {
                tts?.shutdown()
                tts = null
    ...
    private fun loadNextChapterForTts() {
        ...
        Logger.d { "TTS: Auto-loading next chapter for playback" }
    ...
    fun speak(text: String) {
        if (!ttsInitialized || tts == null) {
            Logger.w { "TTS not initialized, cannot speak" }
            return
        }
    ...
    private fun ensureTtsInitialized() {
        if (tts == null) {
            try {
                ...
                Logger.d { "TTS: Initialization started${if (engineName != null) " with engine $engineName" else ""}" }
            } catch (e: Exception) {
                Logger.e { "TTS: Failed to create TextToSpeech instance: ${e.message}" }
            }
        }
    }
    ...
    fun startTts() {
        ...
        if (!ttsInitialized) {
            Logger.w { "TTS: Not initialized yet, waiting..." }
        ...
        Logger.d { "TTS: Starting to speak ${text.length} characters" }
        speak(text)
    }
    ...
    fun startTtsFromViewport() {
        ...
        if (!ttsInitialized) {
            Logger.w { "TTS: Not initialized yet" }
        ...
        if (text.isNullOrEmpty()) {
            Logger.w { "TTS: No text available for viewport start" }
            return
        }

        // Find first paragraph visible in viewport
        val firstVisibleParagraphIndex = findFirstVisibleParagraphIndex(text)
        Logger.d { "TTS: Starting from viewport paragraph $firstVisibleParagraphIndex" }
    ...
    override fun setChapters(chapters: ViewerChapters) {
        ...
        loadJob = scope.launch {
            ...
            if (loader == null) {
                Logger.e { "NovelViewer: No page loader available" }
                hideLoadingIndicator()
                return@launch
            }
            ...
            page.statusFlow.collectLatest { state ->
                when (state) {
                    ...
                    is Page.State.Error -> {
                        hideLoadingIndicator()
                        displayError(state.error, onRetry = {
    ...
    private fun displayChapter(chapter: ReaderChapter, page: ReaderPage) {
        ...
        if (content.isNullOrBlank()) {
            Logger.e { "NovelViewer: Page text is null or blank" }
            displayError(
                Exception(activity.getString(MR.strings.novel_error_no_text)),
                onRetry = {
                    val loader = chapter.pageLoader ?: return@displayError
                    loader.retryPage(page)
                    scope.launch(Dispatchers.IO) { loader.loadPage(page) }
                },
            )
    ...
    private fun cleanupDistantChapters() {
        ...
        while (loadedChapters.size > maxChapters && currentChapterIndex > 0) {
            ...
            Logger.d { "NovelViewer: Removed distant chapter, adjusted scroll by -$removedHeight" }
        }
    }
                ...
                override fun onGlobalLayout() {
                    ...
                    if (totalHeight <= 0) {
                        ...
                        scrollView.postDelayed({
                            ...
                            Logger.d { "NovelViewer: Scroll restored (delayed) to ${(progress * 100).toInt()}%" }
                            isRestoringScroll = false
                        }, 200)
                        return
                    }
                    ...
                    isRestoringScroll = true
                    setScrollProgress(progress.coerceIn(0f, 1f))
                    Logger.d { "NovelViewer: Scroll restored to ${(progress * 100).toInt()}%" }
                    isRestoringScroll = false
                }
            })
        } else {
    ...
    private fun applyTextViewStyles(textView: TextView) {
        ...
        textView.setLinkTextColor(finalTextColor)
        // Source HTML often ships its own per-paragraph `ForegroundColorSpan`s (e.g. from
        // inline `<span style="color:#xxx">` markup). Those override `setTextColor` and
        // leave the chapter looking unchanged when the user switches theme. Strip them so
        // the new theme colour actually wins. TTS highlights re-add their own spans on
        // the next utterance, so this is non-destructive.
    ...
    private fun showLoadingIndicator() {
        contentContainer.removeAllViews()
        // Resolve the chapter's effective background color so the M3 LoadingIndicator's
        // surface blends with the rendered novel theme from the first frame (otherwise
        // the user sees a flash of unthemed background before the chapter takes over).
        val theme = preferences.novelTheme.get()
        val (themeBg, _) = getThemeColors(theme)
        val customBg = preferences.novelBackgroundColor.get()
        val bg = if (theme == "custom" && customBg != 0) customBg else themeBg

        // Paint the surrounding container first so the loader's themed Box never has a
        // mismatched seam against the scroll view.
        applyBackgroundColor()
        initialLoadingView = hayai.novel.reader.ui.buildNovelLoadingComposeView(
            context = activity,
            backgroundArgb = bg,
        )
    ...
    private fun displayError(error: Throwable, onRetry: (() -> Unit)? = null) {
        ...
        val errorView = TextView(activity).apply {
            text = "${activity.getString(MR.strings.novel_error_loading_chapter)}: ${error.message ?: ""}"
        ...
        if (onRetry != null) {
            val retryButton = com.google.android.material.button.MaterialButton(activity).apply {
                text = activity.getString(MR.strings.retry)
    ...
    /**
     * Handle the "Remember" action from text selection menu — saves a [hayai.novel.reader.quote.Quote]
     * for the active novel via [hayai.novel.reader.quote.QuoteManager].
     */
    private fun onRememberSelectedText() {
        ...
        if (selectedText != null && chapterName != null && novelId != null) {
