...
/**
 * Manga reader preference screen. Replaces the previous monolithic
 * `SettingsReaderController`; the actual preference DSL is delegated to the
 * file-level [populateMangaReaderPreferences] extension so the new
 ...
 *
 * Every preference key is preserved verbatim — no migration.
 */
class SettingsMangaReaderController : SettingsLegacyController() {

    override fun setupPreferenceScreen(screen: PreferenceScreen) = screen.apply {
        titleRes = MR.strings.manga_reader
        populateMangaReaderPreferences(this)
    }
}
