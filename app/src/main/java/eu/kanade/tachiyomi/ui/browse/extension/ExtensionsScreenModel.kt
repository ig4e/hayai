    ...
    fun searchQueryPredicate(query: String): (Extension) -> Boolean {
        ...
        return { extension ->
            subqueries.any { subquery ->
                if (containsFuzzy(extension.name, subquery)) return@any true

                when (extension) {
                    is Extension.Installed -> extension.sources.any { source ->
                        containsFuzzy(source.name, subquery) ||
                            (source as? HttpSource)?.baseUrl?.let { containsFuzzy(it, subquery) } == true ||
                            source.id == subquery.toLongOrNull()
                    }

                    is Extension.Available -> extension.sources.any {
                        containsFuzzy(it.name, subquery) ||
                            containsFuzzy(it.baseUrl, subquery) ||
                            it.id == subquery.toLongOrNull()
                    }

                    is Extension.Untrusted -> false
                }
            }
        }
    }
