...
class MyAnimeListApi(
    ...
    private fun parseSearchItem(searchResult: eu.kanade.tachiyomi.data.track.myanimelist.dto.MALSearchResultNode): TrackSearch {
        return parseSearchItem(searchResult.node)
    }
    ...
    private fun parseSearchItem(searchItem: MALManga): TrackSearch {
        return TrackSearch.create(trackId).apply {
            ...
            artists = searchItem.authors
                .filter { authorEntry -> authorEntry.role == "Art" }
                .mapNotNull { authorEntry -> authorEntry.node.getFullName() }
            authors = searchItem.authors
                .filter { authorEntry -> authorEntry.role.contains("Story") }
                .mapNotNull { authorEntry -> authorEntry.node.getFullName() }
        }
    }
