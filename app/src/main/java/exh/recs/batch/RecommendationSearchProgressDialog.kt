        ...
        value = when (status) {
            is SearchStatus.Initializing -> {
                RecommendationSearchProgressProperties(
                    title = context.getString(MR.strings.rec_collecting),
                    text = context.getString(MR.strings.rec_initializing),
                    negativeButtonText = context.getString(MR.strings.action_cancel),
                    negativeButton = setStatusCancelling,
                )
            }
            is SearchStatus.Error -> {
                RecommendationSearchProgressProperties(
                    title = context.getString(MR.strings.rec_error_title),
                    text = context.getString(MR.strings.rec_error_string, status.message),
                    positiveButtonText = context.getString(MR.strings.action_ok),
            ...
            is SearchStatus.Processing -> {
                RecommendationSearchProgressProperties(
                    title = context.getString(MR.strings.rec_collecting),
                    text = context.getString(MR.strings.rec_processing_state, status.current, status.total) + "\n\n" + status.manga.title,
                    negativeButtonText = context.getString(MR.strings.action_cancel),
                    negativeButton = setStatusCancelling,
                )
            }
