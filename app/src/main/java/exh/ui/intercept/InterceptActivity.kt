...
class InterceptActivity : BaseActivity<Nothing>() {
            ...
            Column(
                ...
                when (status) {
                    ...
                    is InterceptResult.Success -> Text(
                        text = getString(MR.strings.launching_app),
                        style = MaterialTheme.typography.titleLarge,
                    )
                ...
                when (it) {
                    ...
                    is InterceptResult.Failure -> {
                        MaterialAlertDialogBuilder(this)
                            .setTitle(getString(MR.strings.chapter_error))
                            .setMessage(getString(MR.strings.could_not_open_entry))
                            .setPositiveButton(getString(MR.strings.action_ok), null)
            ...
            if (sources.size > 1) {
                withContext(Dispatchers.Main) {
                    MaterialAlertDialogBuilder(this@InterceptActivity)
                        .setTitle(getString(MR.strings.label_sources))
    ...
    private suspend fun loadGalleryEnd(gallery: String, source: UrlImportableSource? = null) {
        ...
        status.value = when (result) {
            is GalleryAddEvent.Success -> InterceptResult.Success(result.manga.id!!, result.manga)
...
sealed class InterceptResult {
    ...
    data class Success(val mangaId: Long, val manga: Manga) : InterceptResult()
